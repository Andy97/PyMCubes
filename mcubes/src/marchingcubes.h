...
template<typename vector3, typename formula>
void marching_cubes_partial(const vector3& lower, const vector3& upper,
    int numx, int numy, int numz, formula f, double isovalue,
    std::vector<double>& vertices, std::vector<typename vector3::size_type>& polygons)
{
    using coord_type = typename vector3::value_type;
    ...
    for(int i=0; i<numx; ++i)
    {
        ...
        for(int j=0; j<numy; ++j)
        {
            ...
            for(int k=0; k<numz; ++k)
            {
                ...
                for(int m=0; m<8; ++m)
                {
                    if(v[m] < -10000 || v[m] > 10000) //which means sdf is not available here
                        invalid_count++;
                }
                
                if(invalid_count == 8) //which means all 8 values are not available
                    continue;
